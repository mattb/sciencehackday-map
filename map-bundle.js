...
(function map() {
  ...
  d3.labeler = function l() {
    ...
    var w_inter = 1.0; // leader line intersection
    var w_lab2 = 30.0; // label-label overlap
    var w_lab_anc = 30.0; // label-anchor overlap
    var w_orient = 3.0; // orientation bias
    ...
    function energy(index) {
      ...
      var m = lab.length;
      var ener = 0;
      var dx = lab[index].x - anc[index].x;
      var dy = anc[index].y - lab[index].y;
      ...
      var y11 = void 0;
      var y12 = void 0;
      var x_overlap = void 0;
      var y_overlap = void 0;
      var overlap_area = void 0;
    ...
    var user_defined_schedule = void 0;
    var user_schedule = false;
    var acc = 0;
    var rej = 0;
    ...
    labeler.alt_schedule = function (x) {
      ...
      if (!_arguments.length) return cooling_schedule;
  ...
  var makeMap = function makeMap(id) {
    var elt = d3.select(id);
    elt.html('');
    var eltRect = elt.node().getBoundingClientRect();
    var width = eltRect.width;
    var height = eltRect.height;

    var rollover = d3.select('#map-rollover');
    rollover.style('width', width - 60 + 'px').style('top', height - 100 + 'px');

    var projection = d3.geoMercator().scale(140).rotate([-180, 0]);
    ...
    d3.json('world-simp.json', function (error, topology) {
      var g = svg.append('g');
      var feature = topojson.feature(topology, topology.objects.countries);
      console.log(d3.geoCentroid(feature));
      console.log(d3.geoBounds(feature));
      g.selectAll('path').data(feature.features).enter().append('path').attr('d', path);

      var bounds = path.bounds(feature);
      var dx = bounds[1][0] - bounds[0][0];
      var dy = bounds[1][1] - bounds[0][1];
      var x = (bounds[0][0] + bounds[1][0]) / 2;
      var y = (bounds[0][1] + bounds[1][1]) / 2 - 40;
      var scale = 1.7 / Math.max(dx / width, dy / height);
      var translate = [width / 2 - scale * x, height / 2 - scale * y];
      g.attr('transform', 'translate(' + translate + ')scale(' + scale + ')');

      // load and display the cities
      d3.json('flags-with-location.js', function (d3err, data) {
        var markers = [];
        var labels = [];
        var seen = {};
        var previous = {};
        ...
        d3.select('#rollover-initial').html(counters.events + ' events, ' + counters.cities + ' cities, ' + counters.countries + ' countries');
        d3.select('#rollover-upcoming').html('');
        d3.select('#rollover-previous').html('');

        var lines = g.append('g');

        g.selectAll('circle').data(markers).enter().append('circle').attr('id', function (d, i) {
          return 'circle-' + i;
        }).attr('cx', function (d) {
          return d.x;
        }).attr('cy', function (d) {
          return d.y;
        }).attr('r', 6).style('opacity', '0.7').style('fill', '#999');

        var labelTexts = g.selectAll('text').data(labels).enter().append('text').classed('map-text', true).attr('text-anchor', function (d) {
        ...
        labelTexts.on('mouseover', function (d, i, nodes) {
          g.selectAll('text').transition().duration(250).style('font-size', '12px').style('opacity', '0.1');
          ...
          d3.select(nodes[i]).transition().duration(250).style('font-size', '20px').style('opacity', '1.0');
        });
        labelTexts.on('mouseout', function (d, i) {
          g.selectAll('text').transition().duration(250).style('font-size', '12px').style('opacity', '0.7');
          g.selectAll('circle').transition().duration(250).style('opacity', '0.7');
          d3.select('#line-' + i).transition().style('opacity', '0.0');
        });

        var index = 0;
        labelTexts.each(function (d, i, nodes) {
        ...
        process.nextTick(function () {
          d3.labeler().label(labels).anchor(markers).width(width).height(height).start(500);
          svg.selectAll('text.map-text').style('opacity', '0.8').attr('x', function (d) {
            return d.x;
          }).attr('y', function (d) {
            return d.y;
          }).attr('dx', function () {
          ...
          lines.selectAll('line').data(labels).enter().append('line').attr('x1', function (d) {
            return d.x - 2;
          }).attr('y1', function (d) {
            return d.y;
            ...
            return markers[i].y;
          }).style('opacity', '0.0').attr('stroke', '#999').attr('id', function (d, i) {
            return 'line-' + i;
...
},{"_process":8,"d3":5,"d3-geo-projection":3,"moment":6,"topojson":7}],2:[function(require,module,exports){
...
},{"d3-array":2,"d3-geo":4}],4:[function(require,module,exports){
...
},{"d3-array":2}],5:[function(require,module,exports){
